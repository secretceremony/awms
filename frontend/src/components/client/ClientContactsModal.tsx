...
import { Modal, FormField, Input, Button, StatusBadge, ConfirmModal } from '../ui/index.js';
...
export const ClientContactsModal: React.FC<ClientContactsModalProps> = ({
  ...
  const handleSaveContact = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingContact) {
        ...
        showToast({ type: 'success', message: `Contact "${name}" updated successfully` });
      } else {
        await apiClient.post(`/clients/${client.id}/contacts`, payload);
        showToast({ type: 'success', message: `Contact "${name}" added successfully` });
      }
  ...
  const handleToggleStatus = async (contact: ClientContact) => {
    ...
    try {
      ...
      showToast({
        type: 'success',
        message: `Contact "${contact.name}" ${contact.isActive ? 'deactivated' : 'reactivated'}`,
      });
      await fetchContacts();
      onUpdate();
    } catch (err: any) {
      console.error(err);
      const msg = err.message || 'Failed to update contact status';
      setErrorMsg(msg);
      showToast({ type: 'error', message: msg });
    }
  };
  ...
  const handleConfirmDelete = async () => {
    ...
    setErrorMsg(null);
    try {
      await apiClient.delete(`/clients/${client.id}/contacts/${contactToDelete.id}`);
      showToast({ type: 'success', message: `Contact "${contactToDelete.name}" deleted` });
      setContactToDelete(null);
      await fetchContacts();
      onUpdate();
    } catch (err: any) {
